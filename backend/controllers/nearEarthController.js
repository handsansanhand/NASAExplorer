...
function extractData(nearEarthObjects) {
  ...
  for (const date in nearEarthObjects) {
    ...
    objectsOnDate.forEach((asteroid) => {
      const close_approach_data = extractCloseApproachData(
      ...
      const asteroidInfo = {
        ...
        "Average Estimated Diameter (Meters)": calculateSize(
...
function calculateSize(estimated_diameter) {
  ...
  return (
    (meters.estimated_diameter_min + meters.estimated_diameter_max) /
    2
  ).toFixed(2);
}
...
*/
async function getTimelineOfAsteroid(req, res) {
  console.log(`Retrieving timeline of asteroid...`);
  try {
    const { id } = req.params;
    ...
    if (!request.ok) {
      const errorText = await request.json();
      console.error(
        `There was a server side problem when retrieving asteroid timeline.`
      );
      res.status(500).json({ status: request.status, message: errorText });
      return;
    }
    const data = await request.json();
    ...
    closeApproachData.forEach((approach) => {
      ...
      const approachJSON = {
        ...
        Speed: Number(relative_velocity.kilometers_per_hour).toFixed(2),
        "Miss Distance": Number(miss_distance.kilometers).toFixed(2),
      };
